...
var { basicVer, imageQuery } = require('./middleware')
...
router.post('/', basicVer, imageQuery, async (req, res) => {
  try {
    let { user, sessionToken } = req
    let { toAccountWords } = req.user
    let { license, freeImage } = user.attributes
    ...
    if (!needToConsume) {
      ...
      if (license) {
        try {
          console.log('扣免费次数')
          // 尝试扣免费次数
          user.increment('freeImage', -1)
          await user.save(null, {
            query: new AV.Query(AV.User).greaterThanOrEqualTo('freeImage', 1),
            useMasterKey: true
          })
          describe += '扣免费次数'
        } catch (e) {
          console.log('扣余额')
          // 扣余额
          let count = Number(process.env.imageConsume)
          let user = await AV.User.become(req.headers['x-lc-session'])
          user.increment('balance', -count)
          await user.save(null, {
            query: new AV.Query(AV.User).greaterThanOrEqualTo('balance', count),
            useMasterKey: true
          })
          describe += `扣余额${count}`
        }
      // 在试用期
        ...
        // 扣试用次数
        console.log('扣试用次数')
        user.increment('probationImage', -1)
        await user.save(null, {
          query: new AV.Query(AV.User).greaterThanOrEqualTo('probationImage', 1),
          useMasterKey: true
        })
        describe += '扣试用次数'
      }
